...
## 下载
Github（国际） 下载 [https://github.com/anjoy8/lgs.web.api](https://github.com/anjoy8/lgs.web.api)  
    
Gitee（国内） 下载 [https://gitee.com/laozhangIsPhi/lgs.web.api](https://gitee.com/laozhangIsPhi/lgs.web.api)  


## 编译与运行
1、拿到项目后，双击 `lgs.web.api.sln` 解决方案；  
2、首先 `F6` 编译，看是否有错误；  
...
1、打开 `lgs.web.api` 项目下的 `appsettings.json` 文件；  
...
## 如何配置项目端口号
1、在 `lgs.web.api` 层下的 `program.cs` 文件中，将 `8081`端口，修改为自己想要的端口号；    
...
## 新增实体模块后如何迁移到数据库
1、在 `lgs.web.api.Model` 项目目录下的 `Seed` 文件夹下，找到 `DBSeed` 类；  
...
## 新增数据库表，如何反向生成四层文件
1、可以通过 `T4` 模板来生成，在 `lgs.web.api.FrameWork` 层，使用方法: [9757999.html](https://www.cnblogs.com/laozhang-is-phi/p/9757999.html#autoid-4-3-0) ；  
...
## 发布与部署
1、双击项目根目录下的 `lgs.web.api.Publish.bat`批处理文件；  
